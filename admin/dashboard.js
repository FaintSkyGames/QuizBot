...
app.use(
  upload.fields([{ name: "new-bot-avatar" }, { name: "new-bot-banner" }])
);
...
app.get("/leaderboard", async (req, res) => {
  const style = req.query.style || "default"; // e.g. ?style=neon
  ...
  fs.readFile(filePath, "utf8", (err, html) => {
    ...
    // Replace a placeholder with the stylesheet path
    const modifiedHtml = html.replace("{{STYLE}}", `/boards/${style}.css`);
...
// TODO: edit to be overall not a select quiz
app.get("/leaderboard/json", async (req, res) => {

    try {
      // Get all current quiz scores
      const currentQuiz = await quizCurrentSchema.find().lean();

      // Collect the userIds in play
      const userIds = currentQuiz.map(q => q.userId);

      // Fetch matching player records
      const players = await quizPlayersSchema.find({ userId: { $in: userIds } }).lean();

      // Map players by userId for quick lookup
      const playerMap = new Map(players.map(p => [p.userId, p]));

      // Build leaderboard using *only* currentQuiz points
      const leaderboard = currentQuiz.map(score => {
        const player = playerMap.get(score.userId) || {};
        return {
          userId: score.userId,
          name: player.name || player.username || "Unknown",
          avatar: player.avatar || null,
          points: player.totalPoints, // ✅ correct source (quizCurrentSchema)
        };
      });
...
app.get("/scoreboard", async (req, res) => {
  const style = req.query.style || "default"; // e.g. ?style=neon
  const filePath = path.join(__dirname, "views", "scoreboard.html");
    ...
    try {
      ...
      const leaderboard = currentQuiz.map(score => {
        ...
        return {
          ...
          name: player.name || player.username || "Unknown",
          avatar: player.avatar || null,
          points: score.points, // ✅ correct source (quizCurrentSchema)
        };
      });
...
app.post("/api/update-bot", async (req, res) => {
  ...
  if (avatarFile) {
    try {
      ...
      return res.json({
        success: false,
        message: "Failed to update bot avatar",
      });
    }
  }
  ...
  if (bannerFile) {
    try {
      ...
      return res.json({
        success: false,
        message: "Failed to update bot banner",
      });
    }
  }
...
app.get("/api/bot-stats", async (req, res) => {
  try {
    ...
    const totalCommands =
      (client.commands ? client.commands.size : 0) +
      (client.prefix ? client.prefix.size : 0);
    const botName = client.user.username;
...
app.post("/update-config", (req, res) => {
  ...
  const newConfig = {
    bot: {
      ...
      ...(req.body.developerCommandsServerIds && {
        developerCommandsServerIds:
          req.body.developerCommandsServerIds.split(","),
    ...
    logging: {
      ...
      ...(req.body.guildJoinLogsId && {
        guildJoinLogsId: req.body.guildJoinLogsId,
      }),
      ...(req.body.guildLeaveLogsId && {
        guildLeaveLogsId: req.body.guildLeaveLogsId,
      }),
      ...(req.body.commandLogsChannelId && {
        commandLogsChannelId: req.body.commandLogsChannelId,
      }),
          ...
          return new Promise((resolveFile, rejectFile) => {
            fs.stat(filePath, (err, stats) => {
              ...
              if (stats.isDirectory()) {
                readCommands(filePath, commandArray, type)
                  .then(resolveFile)
                  .catch(rejectFile);
...
app.get("/api/errors", (req, res) => {
  ...
  fs.readdir(errorsDir, (err, files) => {
    ...
    files.sort(
      (a, b) =>
        fs.statSync(path.join(errorsDir, b)).mtime -
        fs.statSync(path.join(errorsDir, a)).mtime
    );
